        ...
        if self.ser:
            try:
                ...
                if self.send_hex_cbtn_var.get() == 1:
                    self.ser.write(bytes(send_data,encoding='utf-8'), isHex=True)
                else:
                    self.ser.write(bytes(send_data,encoding='utf-8'))
            except Exception as e:
    ...
    def SerialRead(self):
        ...
        while self.ser.alive:
            try:
                ...
                if n:
                    self.receive_data += str(self.ser.l_serial.read(n),encoding='utf-8')
